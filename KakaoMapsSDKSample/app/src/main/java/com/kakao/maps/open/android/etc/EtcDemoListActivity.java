...
public class EtcDemoListActivity extends AppCompatActivity implements SubListAdapter.OnItemClickListener {

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_sub_list);
        setTitle("Etc Demo List");

        LinearLayoutManager manager = new LinearLayoutManager(this);
        SubListAdapter adapter = new SubListAdapter();
        adapter.setOnClickListener(this);
        adapter.setItems(new SubListAdapter.SubListItem("지도 캡쳐하기",
                "지도화면을 이미지 파일로 만들기", MapCaptureDemoActivity.class),
                        new SubListAdapter.SubListItem("CalloutBalloon 스타일 만들기",
                "CalloutBalloon 스타일의 라벨 만들기", CalloutBalloonStyleDemoActivity.class));
